...
contract('Exchange', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    ex = await Exchange.new(swReg.address, {gasPrice:0});
  });
  ...
  it("test buy after sell orders", async function () {
    ...
    assertBnEq(web3.toWei(0.6, 'finney'), await web3.eth.getBalance(swReg.address));
    ...
    assertBnEq(beforeRedeem2.add(web3.toWei(0.6, 'finney')), await web3.eth.getBalance(role.trader1));
  ...
  it("test buy after sell orders without price and than sell", async function () {
    ...
    //sell 1 kovan finney with price 0.7 ether for 1 kovan ether
    ...
    await ex.addHash(secretHash, {from: role.trader3});
    await ex.addHash(secretHash2, {from: role.trader4});
    ...
    assertBnEq(web3.toWei(999.50, 'finney'), await ex.myDeposit({from: role.trader4}));
    assertBnEq(web3.toWei(3999.50, 'finney'), await web3.eth.getBalance(ex.address));
